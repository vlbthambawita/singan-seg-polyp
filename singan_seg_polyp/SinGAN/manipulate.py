...
def SinGAN_generate_clean(Gs,Zs,reals,NoiseAmp,opt,in_s=None,scale_v=1,scale_h=1,n=0,gen_start_scale=0,num_samples=50):
    #if torch.is_tensor(in_s) == False:
    if in_s is None:
        in_s = torch.full(reals[0].shape, 0, device=opt.device, dtype=torch.bool)
    images_cur = []
        ...
        for i in range(0,num_samples,1):
            ...
            if n == len(reals)-1:
                ...
                if (opt.mode != "harmonization") & (opt.mode != "editing") & (opt.mode != "SR") & (opt.mode != "paint2image"):
                    ...
                    img_path = '%s/chk_id_%s_gen_scale_%d_%d_img.png' % (opt.dir2save, opt.checkpoint_id , opt.gen_start_scale, i)
                    mask_path = '%s/chk_id_%s_gen_scale_%d_%d_mask.png' % (opt.dir2save, opt.checkpoint_id , opt.gen_start_scale, i)
                    ...
                    if opt.mask_post_processing:
                        ...
                        mask = np.dstack([mask]*3) # to make sure all channels are same in mask
                        vmax = 255
                    
                    plt.imsave(img_path, 
                               functions.convert_image_np(I_curr.detach())[:,:,0:3], vmin=0,vmax=vmax) # Vajira
