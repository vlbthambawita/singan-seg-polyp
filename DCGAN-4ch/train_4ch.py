...
parser.add_argument('--dataset', help='cifar10 | lsun | mnist |imagenet | folder | lfw | fake | polyp', default="polyp")
parser.add_argument('--dataroot', required=False, help='path to dataset', default="/work/vajira/DATA/kvasir_seg/real_images_root/real_images")
...
if opt.dataset in ['imagenet', 'folder', 'lfw']:
    ...
    dataset = ImageAndMaskDataFromSinGAN(opt.path_img, opt.path_mask, transform=trans)
    nc=opt.nc
...
class Discriminator(nn.Module):
    ...
    def forward(self, input):
        if input.is_cuda and self.ngpu > 1:
            ...
            output = self.main(input)
           # print("output shapee=", output.shape)
