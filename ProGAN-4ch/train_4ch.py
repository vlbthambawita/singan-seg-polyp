...
from progan_modules_4ch import Generator, Discriminator
from custom_data import ImageAndMaskData
...
def imagefolder_loader_img_mask(path_img, path_mask):
    def loader(transform):
        #data = datasets.ImageFolder(path, transform=transform)
        data = ImageAndMaskData(path_img, path_mask, transform=transform)
...
def imagefolder_loader(path):
    def loader(transform):
        data = datasets.ImageFolder(path, transform=transform)
        #data = ImageAndMaskData(path, transform=transform)
        data_loader = DataLoader(data, shuffle=True, batch_size=batch_size,
                                 num_workers=4)
        return data_loader
    return loader
...
def sample_data(dataloader, image_size=4):
    transform = transforms.Compose([
        ...
        #transforms.Grayscale(num_output_channels=1), # just for testing
        transforms.ToTensor(),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img_dir', type=str, default="/work/vajira/DATA/kvasir_seg/real_images_root/real_images", help="Image directory")
    parser.add_argument("--mask_dir", type=str, default= "/work/vajira/DATA/kvasir_seg/real_masks_root/real_masks", help = "mask directory")
    ...
    generator = Generator(in_channel=args.channel, input_code_dim=input_code_size, pixel_norm=args.pixel_norm, tanh=args.tanh, img_channels= args.img_channels).to(device)
    discriminator = Discriminator(feat_dim=args.channel, img_channels = args.img_channels).to(device)
    g_running = Generator(in_channel=args.channel, input_code_dim=input_code_size, pixel_norm=args.pixel_norm, tanh=args.tanh, img_channels = args.img_channels).to(device)
    ...
    loader = imagefolder_loader_img_mask(args.img_dir, args.mask_dir)
